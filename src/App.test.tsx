...
test('renders the maternity pay dashboard', () => {
  ...
  expect(screen.getByLabelText(/contingency buffer/i)).toBeInTheDocument()
  expect(screen.getByText(/total estimated take-home/i)).toBeInTheDocument()
  expect(screen.getByText(/recommended reserve/i)).toBeInTheDocument()
  expect(screen.getByText(/contingency included/i)).toBeInTheDocument()
  ...
  expect(screen.getByLabelText(/august 2026 shortfall after buffer/i)).toBeInTheDocument()
...
test('copies the monthly breakdown as csv', async () => {
  ...
  expect(writeText).toHaveBeenCalledWith(
    expect.stringContaining(
      'Month,Leave days,Rate mix,Estimated take-home pay,Monthly commitments,Contingency buffer,Planning target',
    ),
  )
